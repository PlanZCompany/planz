...
const Tech = ({ locale }: { locale: Locale }) => {
  ...
  const featuresContent = TECH_BENEFITS.map((feature, index) => (
    <li
      ...
      className="md:px-4 py-2 flex justify-center items-start md:items-center gap-2 w-full md:w-fit"
    >
  ...
  const imagesContent = TECH_FEATURE_IMAGES.map((image, index) => {
    const borderLeft =
      index !== 2 && index !== 5 && index !== 8 && index !== 11
        ? "md:border-r-3"
        : "";
    const borderBottom = index <= 8 ? "md:border-b-3" : "";

    return (
      <li
        key={`${image.id}`}
        className={`flex-1 p-3 md:p-6 min-h-[88px] md:min-h-[unset] border-r-primaryDarkGreen border-b-primaryDarkGreen ${borderLeft} ${borderBottom}`}
      >
        <GenericImage
          src={image.image.src}
          alt={image.image.alt}
  ...
  return (
    <section className="w-full flex scroll-mt-20 pt-10 md:pt-20" id="tech">
